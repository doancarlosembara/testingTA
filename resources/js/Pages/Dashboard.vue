...
const submit = () => {
    ...
    const swalWithBootstrapButtons = Swal.mixin({
        customClass: {
            ...
            confirmButton:
                "text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900",
        },
    ...
    swalWithBootstrapButtons
        ...
        .then((result) => {
            if (result.isConfirmed) {
                form.post(
                    route(postRoute, { id: usePage().props.auth.user.id }),
                    {
                        preserveScroll: true,
                        onError: () => {
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="flex justify-between gap-4">
                    ...
                    <Card class="w-full md:w-1/4 !mt-0" vl :fancy-p="false">
                        ...
                        <PrimaryButton
                            ...
                            disabled
                        >
                            <span v-if="is_today_off" class="text-sm"
                                >{{ __("Today is off! 🕺🕺") }}<br
                            /></span>
                            <span v-else class="text-sm"
                                >{{ __("Attendance Taken Today! 🎉") }}<br
                            /></span>
                        </PrimaryButton>
                    </Card>
                </div>
                ...
                <div class="flex flex-col md:flex-row justify-between md:gap-4">
                    <!-- PAY DAY -->
                    <Card
                        class="w-full md:w-1/4 bg-gradient-to-r from-blue-700 to-blue-800 glow-element"
                    >
                        ...
                        <div class="mt-4 text-white">
                            <p class="text-white">
                                <span class="font-medium"
                                    >{{ __("Pay Day") }}: </span
                                >{{ props.payroll_day }}<sup>{{ dayNth }}</sup>
                                {{ __("of every month") }}.
                            </p>
                            <p class="mt-1 text-white">
                                <span class="font-medium"
                                    >{{ __("Days Remaining") }}: </span
                                >{{ days_remaining }} {{ __("Days") }}.
                            </p>
                            <!-- Custom ProgressBar -->
                            <div
                            ...
                            >
                                <div
                                    class="bg-amber-400 text-xs font-medium text-blue-700 text-center p-0.5 leading-none rounded-full"
                                    :style="
                                        'width:' +
                                        (pay_day_percentage > 100
                                            ? 100
                                            : pay_day_percentage) +
                                        '%'
                                    "
                    ...
                    <Card class="w-full md:w-1/4">
                        ...
                        <div class="mt-4 w-full flex flex-col">
                            <div
                                class="flex justify-between align-middle mb-6 sm:mb-2"
                            >
                                <p class="font-medium">
                                    {{ __("Work Days") }}:
                                </p>
                            ...
                            >
                                ...
                                <p>
                                    {{ employee_stats["weekendsThisMonth"] }}
                                    {{ __("Days") }}
                                </p>
                            </div>
                            <div
                            ...
                            >
                                ...
                                <p>
                                    {{ employee_stats["holidaysThisMonth"] }}
                                    {{ __("Days") }}
                                </p>
                            </div>
                        </div>
                    ...
                    <Card class="w-full md:w-2/4">
                        <h1 class="text-2xl">
                            {{ __("Your Attendance This Month") }}
                        </h1>
                        <div class="mt-4 grid grid-rows-3">
                            <div
                                class="flex flex-col lg:flex-row justify-between align-middle mb-6 sm:mb-2"
                            >
                                ...
                                <ProgressBar
                                    ...
                                    "
                                    :text="
                                        employee_stats['totalAbsenceSoFar'] +
                                        (employee_stats['totalAbsenceSoFar'] > 0
                                            ? __('Day(s)')
                                            : '')
                                    "
                                />
                            ...
                            >
                                <p class="w-full sm:w-1/3">
                                    {{ __("Absented:") }}
                                    {{ employee_stats["absentedThisMonth"] }}
                                </p>
                                <ProgressBar
                                    no-text
                                    color="bg-red-500"
                ...
                <div class="flex flex-col md:flex-row justify-between md:gap-4">
                    <Card class="!p-2 w-full">
                        ...
                        <div class="flex flex-wrap justify-center gap-4">
                            <Card
                                class="w-full lg:w-1/4 !shadow-none !overflow-visible flex-1"
                                :fancy-p="false"
                            >
                                <IconCard
                                    :heading="__('Payrolls')"
                                    :cta-text="__('Go To Payments')"
...
@keyframes glowing {
    ...
    50% {
        box-shadow: 0 0 10px dodgerblue, 0 0 15px dodgerblue;
    }
    100% {
        box-shadow: 0 0 5px dodgerblue;
    }
